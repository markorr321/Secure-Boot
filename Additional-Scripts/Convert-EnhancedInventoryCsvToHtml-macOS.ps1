...
.SYNOPSIS
    ...
    macOS version - uses native macOS dialogs via osascript.
...
.EXAMPLE
    ./Convert-EnhancedInventoryCsvToHtml-macOS.ps1
    # Opens file picker dialog

.EXAMPLE
    ./Convert-EnhancedInventoryCsvToHtml-macOS.ps1 -CsvPath "~/Downloads/RemediationResults.csv"

.EXAMPLE
    ./Convert-EnhancedInventoryCsvToHtml-macOS.ps1 -CsvPath "./export.csv" -OutputPath "~/Reports"
#>
...
# If no CSV path provided, open file picker using osascript (macOS native)
if (-not $CsvPath) {
    Write-Host "Opening file picker..." -ForegroundColor Cyan
    
    $appleScript = @'
tell application "System Events"
    activate
    set theFile to choose file with prompt "Select Intune Proactive Remediation Export CSV" of type {"csv", "public.comma-separated-values-text"}
    return POSIX path of theFile
end tell
'@
    
    try {
        $CsvPath = osascript -e $appleScript 2>/dev/null
...
if ($CsvPath.StartsWith("~")) {
    $CsvPath = $CsvPath -replace "^~", $env:HOME
}
...
# Expand ~ in output path if present
if ($OutputPath.StartsWith("~")) {
    $OutputPath = $OutputPath -replace "^~", $env:HOME
}
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f3f2f1; }
        ...
        .json-code { background: #f3f2f1; padding: 12px; border-radius: 4px; font-family: 'SF Mono', 'Menlo', 'Consolas', monospace; font-size: 11px; white-space: pre-wrap; word-break: break-all; max-height: 300px; overflow-y: auto; }
    </style>
</head>
<body>
<div class="container">
